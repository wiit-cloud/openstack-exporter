...
var defaultMasakariMetrics = []Metric{
	...
	{Name: "host", Labels: []string{"id", "uuid", "name", "failover_segment_id", "failover_segment_name", "type", "control_attributes"}, Fn: nil},
	{Name: "host_on_maintenance", Labels: []string{"uuid", "name", "failover_segment_id"}, Fn: nil},
	{Name: "host_reserved", Labels: []string{"uuid", "name", "failover_segment_id"}, Fn: nil},
}
	...
	for _, segment := range segments {
		...
		for _, host := range hosts {
			ch <- prometheus.MustNewConstMetric(exporter.Metrics["host"].Metric,
				prometheus.GaugeValue, 1.0,
				strconv.Itoa(host.ID), host.UUID, host.Name, host.FailoverSegmentID,
